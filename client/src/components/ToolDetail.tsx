...
export function ToolDetail({ name, tagline, description, accentColor, icon: Icon, features, plans, path: pathProp, specTables, useCases, competitorRows, extendedFaqs, deepDive }: ToolDetailProps) {
  ...
  const autoFaqs = [
    { q: `How much does ${name} cost in Bangladesh?`, a: `${name} starts at ${cheapestPlan?.price || "৳499"}${cheapestPlan?.period || "/mo"} from AI Team Premium, payable in BDT via bKash or Nagad. No international card needed.` },
